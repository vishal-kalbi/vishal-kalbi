# Vishal Choudhary

**Software Engineer | Backend & Cloud | Open Source Contributor**

Experienced software engineer with a strong background in backend development, cloud deployment, and building AI-powered applications.  
I enjoy working on scalable systems, developer tools, and production-grade software.

---

## Tech Stack

### Languages & Frameworks
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-323330?logo=javascript)
![Django](https://img.shields.io/badge/Django-092E20?logo=django)
![Django REST](https://img.shields.io/badge/Django%20REST-092E20)
![Flask](https://img.shields.io/badge/Flask-black?logo=flask)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?logo=fastapi)
![Scrapy](https://img.shields.io/badge/Scrapy-60A839)

### Frontend
![React](https://img.shields.io/badge/React-20232A?logo=react)
![Next.js](https://img.shields.io/badge/Next.js-black?logo=nextdotjs)

### Databases & Data
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?logo=postgresql)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?logo=mysql)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?logo=mongodb)
![BigQuery](https://img.shields.io/badge/BigQuery-4285F4?logo=googlecloud)
![Vector DB](https://img.shields.io/badge/Vector%20Databases-555555)

### AI & LLM Systems
![LangChain](https://img.shields.io/badge/LangChain-000000)
![LlamaIndex](https://img.shields.io/badge/LlamaIndex-000000)
![RAG](https://img.shields.io/badge/RAG%20Pipelines-333333)

### Cloud & DevOps
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?logo=githubactions)
![AWS](https://img.shields.io/badge/AWS-232F3E?logo=amazonaws)
![Azure](https://img.shields.io/badge/Azure-0078D4?logo=microsoftazure)
![GCP](https://img.shields.io/badge/GCP-4285F4?logo=googlecloud)
![DigitalOcean](https://img.shields.io/badge/DigitalOcean-0080FF?logo=digitalocean)
![Vercel](https://img.shields.io/badge/Vercel-black?logo=vercel)
![Railway](https://img.shields.io/badge/Railway-0B0D0E)

---

## Areas of Work

- Backend architecture & API development  
- AI-powered applications using LLMs (LangChain, LlamaIndex, RAG systems)  
- Data-intensive applications & pipelines  
- Cloud infrastructure & production deployments  
- CI/CD automation and containerization  
- Open source contributions & developer tooling  

---
- Email: vishalch445566@gmail.com
